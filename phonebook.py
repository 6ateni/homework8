...
def transfer_data(source: str, dest: str, num_row: int):
    ...
    """
    with open(source, "r", encoding="utf-8") as firstfile, open(dest,'a', encoding="utf-8") as secondfile:
        list = firstfile.read().split("\n")
        if num_row == 0 or num_row -1> len(list)-1:
            return print("Нет такой строки")
        secondfile.write(list[num_row-1] + "\n")
...
while True:
    ...
    if mode == 1:
        ...
        num_row = int(input("Введите номер переносимой строки: "))
